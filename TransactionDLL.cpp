...
void TransactionDLL::displayAllByPaymentChannel() {
    unordered_set<string> shownChannels;
    Node* current = head;

    while (current) {
        string channel = current->data.payment_channel;

        if (shownChannels.find(channel) == shownChannels.end()) {
...
void TransactionDLL::searchByTransactionType(const string& type) {
    Node* current = head;
    bool found = false;
    ...
    if (!found) {
        cout << "No transaction found for the type: " << type << endl;
    }
}
...
Node* TransactionDLL::getHead() const {
    return head;
}
